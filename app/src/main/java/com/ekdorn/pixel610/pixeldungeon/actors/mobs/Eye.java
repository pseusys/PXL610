...
import com.ekdorn.pixel610.pixeldungeon.effects.particles.PurpleParticle;
...
import com.ekdorn.pixel610.pixeldungeon.items.weapon.enchantments.Leech;
import com.ekdorn.pixel610.pixeldungeon.mechanics.Ballistica;
import com.ekdorn.pixel610.pixeldungeon.sprites.CharSprite;
import com.ekdorn.pixel610.pixeldungeon.sprites.EyeSprite;
...
public class Eye extends Mob {
	...
	{
		name = Babylon.get().getFromResources("mob_eye");
		...
		for (int i=1; i < Ballistica.distance; i++) {
			...
			if (hit( this, ch, true )) {
				...
				if (!ch.isAlive() && ch == Dungeon.hero) {
					...
					GLog.n( Babylon.get().getFromResources("mob_eye_kills"), name );
				}
			} else {
	...
	public String description() {
		return
				Babylon.get().getFromResources("mob_eye_desc");
	}
